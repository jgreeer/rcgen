...
pub use provider::{CryptoProvider, HashAlgorithm};
...
/// KISS function to generate a self signed certificate using the given [`CryptoProvider`].
///
/// Like [`generate_simple_self_signed`], but the key pair is generated by `provider`
/// (using [`PKCS_ECDSA_P256_SHA256`]) and hashing is performed by it too. This works with
/// any backend, including without the `crypto` feature.
...
impl<'a, S: SigningKey> CertifiedIssuer<'a, S> {
	...
	pub fn self_signed(params: CertificateParams, signing_key: S) -> Result<Self, Error> {
		Self::self_signed_with_provider(params, signing_key, &DefaultCryptoProvider)
	}

	/// Create a new issuer from the given parameters and key, with a self-signed certificate,
	/// using `provider` for the hashing needed to derive key identifiers and a default serial
	/// number.
	///
	...
	pub fn signed_by(
		...
		Self::signed_by_with_provider(params, signing_key, issuer, &DefaultCryptoProvider)
	}
...
impl KeyIdMethod {
	...
	pub(crate) fn derive(
		...
		let digest = provider.hash(alg, subject_public_key_info.as_ref());
		digest[0..20].to_vec()
	}
}
